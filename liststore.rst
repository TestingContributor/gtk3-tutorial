...
The constructor for the ListStore is::
  ...
  GtkListStore *liststore = gtk_list_store_new(columns, type, type, type...);
...
New rows are added to the ListStore with::
  ...
  gtk_list_store_prepend(liststore, treeiter);
  gtk_list_store_append(liststore, treeiter);
  gtk_list_store_insert(liststore, treeiter, position);
...
Once the row has been added, the data can then be added::
  ...
  gtk_list_store_set(liststore, treeiter, data, -1);
...
Two rows can be swapped by specifying the TreeIter of both with::
  ...
  gtk_list_store_swap(liststore, treeiter1, treeiter2);
...
Rows can also be moved above or below an existing item by calling::
  ...
  gtk_list_store_move_before(liststore, treeiter, position);
  gtk_list_store_move_after(liststore, treeiter, position);
